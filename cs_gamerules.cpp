...
ConVar mp_consecutive_loss_aversion(
...
ConVar cl_autohelp(
	...
	extern ConVar ff_damage_decoy_explosion;
	...
	CCSGameRules::CCSGameRules()
	{
		...
		m_bConsecutiveCTWin = false;
		m_bConsecutiveTerroristWin = false;
	...
	void CCSGameRules::RadiusDamage( const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore, bool bIgnoreWorld )
	{
        ...
        bool isDecoy = dynamic_cast< CDecoyProjectile* >( pInflictor ) != NULL;        
		...
		for ( CEntitySphereQuery sphere( vecSrc, flRadius ); ( pEntity = sphere.GetCurrentEntity() ) != NULL; sphere.NextEntity() )
		{
			...
			if (pCSExplosionVictim)
			{
				...
				if ( isDecoy && !pCSExplosionVictim->IsOtherEnemy( pCSExplosionAttacker ) && !ff_damage_decoy_explosion.GetBool() )
					continue;
			}
...
//		}
	...
	void CCSGameRules::RoundWin( void )
	{
		...
		int iLoserBonus = TeamCashAwardValue( TeamCashAward::LOSER_BONUS );
		if ( m_iRoundWinStatus == WINNER_TER ) // terrorists won
		{
			iLoserBonus += TeamCashAwardValue( TeamCashAward::LOSER_BONUS_CONSECUTIVE_ROUNDS ) * m_iNumConsecutiveCTLoses;
			if ( mp_consecutive_loss_aversion.GetInt() == 1 )
			{
				if ( m_iNumConsecutiveTerroristLoses > 0 )
					m_iNumConsecutiveTerroristLoses--;
			}
			else if ( mp_consecutive_loss_aversion.GetInt() == 2 )
			{
				if ( m_bConsecutiveTerroristWin )
				{
					if ( m_iNumConsecutiveTerroristLoses > 0 )
						m_iNumConsecutiveTerroristLoses--;
				}
			...
			if ( m_iNumConsecutiveCTLoses < mp_consecutive_loss_max.GetInt() )
				m_iNumConsecutiveCTLoses++; //increment the number of loses the CTs have had
			m_bConsecutiveTerroristWin = true;
			m_bConsecutiveCTWin = false;
		}
		else if ( m_iRoundWinStatus == WINNER_CT ) // CT Won
		{
			iLoserBonus += TeamCashAwardValue( TeamCashAward::LOSER_BONUS_CONSECUTIVE_ROUNDS ) * m_iNumConsecutiveTerroristLoses;
			if ( mp_consecutive_loss_aversion.GetInt() == 1 )
			{
			...
			if ( m_iNumConsecutiveTerroristLoses < mp_consecutive_loss_max.GetInt() )
				m_iNumConsecutiveTerroristLoses++; //increment the number of loses the Terrorists have had
			m_bConsecutiveCTWin = true;
			m_bConsecutiveTerroristWin = false;
		}
        ...
        if (m_iRoundWinStatus == WINNER_TER) // terrorists won
        {
            ...
            AddTeamAccount( TEAM_CT, TeamCashAward::LOSER_BONUS, iLoserBonus );
        }
        else if (m_iRoundWinStatus == WINNER_CT) // CT Won
        {
            AddTeamAccount( TEAM_CT, TeamCashAward::HOSTAGE_ALIVE, iRescuedHostageBonus);
			AddTeamAccount( TEAM_TERRORIST, TeamCashAward::LOSER_BONUS, iLoserBonus );
        }
	...
	void CCSGameRules::RestartRound()
	{
		...
		if ( m_bCompleteReset )
		{
			...
			// Reset score info
			m_match.Reset();
			m_iNumConsecutiveTerroristLoses = MIN( mp_starting_losses.GetInt(), mp_consecutive_loss_max.GetInt() );
			m_iNumConsecutiveCTLoses = MIN( mp_starting_losses.GetInt(), mp_consecutive_loss_max.GetInt() );
			m_bConsecutiveCTWin = false;
			m_bConsecutiveTerroristWin = false;
...
// no winners:
	...
	CBaseEntity *CCSGameRules::GetPlayerSpawnSpot( CBasePlayer *pPlayer )
	{
	    ...
	    // drop down to ground
		Vector GroundPos = DropToGround( pPlayer, pSpawnSpot->GetAbsOrigin() + Vector( 0, 0, 16 ), VEC_HULL_MIN + Vector( -4, -4, 0 ), VEC_HULL_MAX + Vector( 4, 4, 0 ) ) + Vector( 0, 0, 8 );
