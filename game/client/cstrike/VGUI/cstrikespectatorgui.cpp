...
CCSSpectatorGUI::CCSSpectatorGUI(IViewPort *pViewPort) : CSpectatorGUI(pViewPort)
{
	...
	m_pBombIcon = NULL;
	m_pPlayerKills = NULL;
    m_pPlayerAssists = NULL;
    m_pPlayerDeaths = NULL;
    m_pPlayerKD = NULL;
...
void CCSSpectatorGUI::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pPlayerKills   = dynamic_cast<Label*>(FindChildByName( "PlayerKillsValue" ));
    m_pPlayerAssists = dynamic_cast<Label*>(FindChildByName( "PlayerAssistsValue" ));
    m_pPlayerDeaths  = dynamic_cast<Label*>(FindChildByName( "PlayerDeathsValue" ));
    m_pPlayerKD      = dynamic_cast<Label*>(FindChildByName( "PlayerKDValue" ));
...
bool CCSSpectatorGUI::NeedsUpdate( void )
{
	...
	// 获取当前应该显示的真实时间（包含冻结时间的逻辑）
	int currentTime = (int)CSGameRules()->GetRoundRemainingTime();
	if ( CSGameRules()->IsFreezePeriod() )
	{
		currentTime = (int)ceil( CSGameRules()->GetRoundStartTime() - gpGlobals->curtime );
	}
	if ( currentTime < 0 ) currentTime = 0;

	// 如果时间发生了变动，通知 VGUI 刷新界面
	if ( m_nLastTime != currentTime )
		return true;
...
void CCSSpectatorGUI::UpdateTimer()
{
	...
	else
	{
		...
		// 1. 先计算出正确的时间
		int newRoundTime = (int)ceil( pRules->GetRoundRemainingTime() );
		if ( pRules->IsFreezePeriod() )
		{
			// 在冻结时间内，计算距离回合正式开始还剩多少秒
			newRoundTime = (int)ceil( pRules->GetRoundStartTime() - gpGlobals->curtime );
		}
		...
		// 触发动画逻辑（当时间重置时）
		if ( m_iRoundTime < newRoundTime && !pRules->IsFreezePeriod() )
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence( "RoundTimerReset" );

		m_iRoundTime = newRoundTime;

		// 2. 关键：将计算好的时间同步给 m_nLastTime，供 NeedsUpdate() 判断使用
		m_nLastTime = m_iRoundTime;

		if ( m_iRoundTime <= 10 )
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence( "RoundTimerLow" );

		// 3. 关键修复：使用 m_iRoundTime 来格式化分和秒，而不是用旧的 m_nLastTime
		wchar_t szText[16];
	...
	if ( m_nLastSpecTarget != pLocalPlayer->GetObserverTarget() )
	{
		...
		if ( m_nLastSpecTarget && m_nLastSpecTarget->IsPlayer() && m_nLastSpecMode > OBS_MODE_FREEZECAM )
		{
			...
			if ( pSpecTarget ) // shouldn't even be possible but just to be sure
			{
				...
				// ==================== [新增：更新目标玩家的 K / A / D / K/D 数据] ====================
				int iTargetIndex = pSpecTarget->entindex();

				// 从 cs_PR (PlayerResource) 获取实时战绩
				int iKills   = cs_PR->GetFrags( iTargetIndex );
				int iAssists = cs_PR->GetAssists( iTargetIndex );
				int iDeaths  = cs_PR->GetDeaths( iTargetIndex );

				// 计算 K/D 比率（防止除以 0）
				float flKD = ( iDeaths > 0 ) ? ( (float)iKills / (float)iDeaths ) : (float)iKills;

				wchar_t wszValueBuffer[16];

				// 更新 Kills 标签
				if ( m_pPlayerKills )
				{
					V_snwprintf( wszValueBuffer, ARRAYSIZE( wszValueBuffer ), L"%d", iKills );
					m_pPlayerKills->SetText( wszValueBuffer );
					m_pPlayerKills->SetVisible( true );
				}

				// 更新 Assists 标签
				if ( m_pPlayerAssists )
				{
					V_snwprintf( wszValueBuffer, ARRAYSIZE( wszValueBuffer ), L"%d", iAssists );
					m_pPlayerAssists->SetText( wszValueBuffer );
					m_pPlayerAssists->SetVisible( true );
				}

				// 更新 Deaths 标签
				if ( m_pPlayerDeaths )
				{
					V_snwprintf( wszValueBuffer, ARRAYSIZE( wszValueBuffer ), L"%d", iDeaths );
					m_pPlayerDeaths->SetText( wszValueBuffer );
					m_pPlayerDeaths->SetVisible( true );
				}

				// 更新 K/D 标签（保留 1 位小数）
				if ( m_pPlayerKD )
				{
					V_snwprintf( wszValueBuffer, ARRAYSIZE( wszValueBuffer ), L"%.1f", flKD );
					m_pPlayerKD->SetText( wszValueBuffer );
					m_pPlayerKD->SetVisible( true );
				}
				// ====================================================================================
			}
		}
		else
		{
			...
			// ==================== [新增：非观战状态隐藏控件] ====================
			if ( m_pPlayerKills )   m_pPlayerKills->SetVisible( false );
			if ( m_pPlayerAssists ) m_pPlayerAssists->SetVisible( false );
			if ( m_pPlayerDeaths )  m_pPlayerDeaths->SetVisible( false );
			if ( m_pPlayerKD )      m_pPlayerKD->SetVisible( false );
			// ====================================================================
		}
	}
...
// 确保 cl_hud_color 可以被访问（如果提示未定义，请取消下面这行的注释）
// extern ConVar cl_hud_color; 
...
CHudLocation::CHudLocation( const char *pName ) : vgui::Label( NULL, "HudLocation", "" ), CHudElement( pName )
{
	...
	// 初始化为一个无效值，确保第一次 Tick 时必定触发颜色更新
	m_iHUDColor = -1; 
}
...
void CHudLocation::OnTick()
{
	...
	if ( m_iHUDColor != cl_hud_color.GetInt() )
	{
		...
		// 因为 CHudLocation 继承自 vgui::Label，直接设置自身的颜色即可
		SetFgColor( clr );
	}
