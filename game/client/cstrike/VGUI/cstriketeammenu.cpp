...
#include <vgui/IVGui.h> // 【新增】引入 IVGui 用于注册 Tick 信号

#ifdef ANDROID
#define TOUCH_DISABLE_ON_TEAMMENU "1"
#else
#define TOUCH_DISABLE_ON_TEAMMENU "0"
#endif

static ConVar touch_disable_on_teammenu( "touch_disable_on_teammenu", TOUCH_DISABLE_ON_TEAMMENU, FCVAR_ARCHIVE, "Set non-zero to disable touch for entering to teammenu", true, 0, true, 1 );

// 【新增】控制倒计时时间的控制台指令（默认10秒）
static ConVar mp_teammenu_time( "mp_teammenu_time", "10", FCVAR_CLIENTDLL | FCVAR_ARCHIVE, "Auto join team countdown time in seconds" );
...
void CCSTeamMenuAgentImage::ApplySettings( KeyValues* inResourceData )
{
	...
	if ( GetParent() )
	{
		m_pTLogo = dynamic_cast<vgui::VectorImagePanel*>( GetParent()->FindChildByName( "TLogo" ) );
		m_pCTLogo = dynamic_cast<vgui::VectorImagePanel*>( GetParent()->FindChildByName( "CTLogo" ) );
	}

	if ( m_pTLogo )
	{
		m_pTLogo->SetAlpha( 76 );
	}
	if ( m_pCTLogo )
	{
		m_pCTLogo->SetAlpha( 76 );
	}
...
void CCSTeamMenuAgentImage::OnCursorEntered()
{
	if ( GetParent() )
	{
		if ( m_nTeamNumber == TEAM_TERRORIST )
		{
			SetSequence( "t_teamselect_confirm01", 0.2f );
		}
		else if ( m_nTeamNumber == TEAM_CT )
		{
...
void CCSTeamMenuAgentImage::Paint()
{
	...
	CViewSetup view;
	view.x = viewport_xpos; 
	view.y = viewport_ypos; 
	view.width = viewport_wide;		
	view.height = viewport_tall;	
...
CCSTeamMenu::CCSTeamMenu( IViewPort* pViewPort ): Frame( NULL, PANEL_TEAM )
{
	...
	// 【新增】倒计时 Label 及 VGUI Tick 注册
	m_pTimerLabel = new Label( this, "TimerLabel", L"" );
	m_pTimerIcon = new Label( this, "TimerIcon", L"" );
	vgui::ivgui()->AddTickSignal( GetVPanel(), 100 ); // 每 100ms 触发一次 OnTick
	m_flAutoJoinTime = 0.0f;
...
void CCSTeamMenu::ShowPanel( bool bShow )
{
	...
	bool bTouchEnable = (touch_disable_on_teammenu.GetInt() != 0);
...
// 【新增】阵营判断与倒计时重置逻辑
		...
		{
			float flTime = mp_teammenu_time.GetFloat();
			if ( flTime > 0.0f )
			{
				float flRemaining = m_flAutoJoinTime - gpGlobals->curtime;
				
				if ( m_flAutoJoinTime == 0.0f || flRemaining < 0.0f || flRemaining > flTime )
				{
					m_flAutoJoinTime = gpGlobals->curtime + flTime; 
				}
				m_pTimerLabel->SetVisible( true );
				m_pTimerIcon->SetVisible( true ); // 【新增】同步显示图标
			}
			else
			{
				m_flAutoJoinTime = 0.0f;
				m_pTimerLabel->SetVisible( false );
				m_pTimerIcon->SetVisible( false ); // 【新增】同步隐藏图标
			}
		}
		else
		{
...
// 【新增】每帧更新倒计时界面的逻辑
void CCSTeamMenu::OnTick()
{
	BaseClass::OnTick();

	// 如果界面没显示或倒计时被禁用（等于0），直接跳过
	if ( !IsVisible() || m_flAutoJoinTime <= 0.0f )
		return;
...
if ( flRemaining <= 0.0f )
	{
		...
		m_flAutoJoinTime = 0.0f;
		m_pTimerLabel->SetVisible( false );
		m_pTimerIcon->SetVisible( false ); // 【新增】倒计时归零时同步隐藏图标
	...
	{
		// 计算总秒数，并转换为分钟和秒
		int iTotalSeconds = (int)flRemaining;
		int iMinutes = iTotalSeconds / 60;
		int iSeconds = iTotalSeconds % 60;

		// 将剩余时间转化为文本更新到 Label 上
		wchar_t wszText[64];
		
		// 注意这里要把 wszTimer 改成上面声明的 wszText
		V_snwprintf( wszText, sizeof( wszText ), L"%.2d : %.2d", iMinutes, iSeconds );
		
		m_pTimerLabel->SetText( wszText );
	}
}