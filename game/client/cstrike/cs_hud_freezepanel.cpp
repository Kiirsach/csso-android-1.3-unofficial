	...
	CHudElement( pElementName )
{
	...
	m_pFreezePanelBackground = new Panel( this, "FreezePanelBackground" );
	m_pKillerName = new Label( this, "KillerName", L"" );
	m_pAvatar = new CAvatarImagePanel( this, "AvatarImage" );
	m_pAvatarBackground = new ImagePanel( this, "AvatarBackground" );
...
void CCSFreezePanel::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	m_pFreezePanelBackground = FindChildByName( "FreezePanelBackground" );
	m_pKillerName = dynamic_cast<Label*>( FindChildByName( "KillerName" ) );
	m_pAvatarBackground = dynamic_cast<ImagePanel*>( FindChildByName( "AvatarBackground" ) );
}
	...
	else if ( Q_strcmp( "show_freezepanel", pEventName ) == 0 )
	{
		...
		if ( pKiller )
		{
			...
			if ( engine->GetPlayerInfo(iKillerIndex, &pi) )
			{
				...
				if ( m_pAvatarBackground )
				{
					m_pAvatarBackground->SetVisible(true);
				}
			}
			else
			{
				if ( m_pAvatarBackground )
				{
					m_pAvatarBackground->SetVisible(false);
				}
			}
		}
	}
...
void CCSFreezePanel::Show()
{
	...
	if ( m_pFreezePanelBackground )
	{
		m_pFreezePanelBackground->SetVisible( true );
	}
	g_pClientMode->GetViewportAnimationController()->StartAnimationSequence( "HUDFreezePanelShow" );
}
...
void CCSFreezePanel::Hide()
{
	...
	if ( m_pFreezePanelBackground )
	{
		m_pFreezePanelBackground->SetVisible( false );
	}
}
