...
void CCSHudWeaponSelection::AddWeapon( C_BaseCombatWeapon *pWeapon, bool bSelected )
{
	...
	// =========================================================================
	// 判断武器来源并处理名称显示（包括别人的枪及暗色名字逻辑）
	// =========================================================================
	wchar_t wszBaseWepName[256];
	if ( pCSWeapon->HasStatTrak() )
	{
		g_pVGuiLocalize->ConstructString( wszBaseWepName, sizeof( wszBaseWepName ), g_pVGuiLocalize->Find( "#Cstrike_WPNHUD_StatTrak" ), 1, g_pVGuiLocalize->Find( pCSWeapon->GetPrintName() ) );
	}
	else
	{
		const wchar_t *pPrintName = g_pVGuiLocalize->Find( pCSWeapon->GetPrintName() );
		if ( pPrintName )
		{
			V_wcsncpy( wszBaseWepName, pPrintName, sizeof( wszBaseWepName ) );
		}
		else
		{
			g_pVGuiLocalize->ConvertANSIToUnicode( pCSWeapon->GetPrintName(), wszBaseWepName, sizeof( wszBaseWepName ) );
		}
	}

	// 检查是否有前主人（丢弃者/初始拥有者）
	// 注意：m_hPrevOwner 是 CHandle<C_BasePlayer> 类型，使用 .Get() 获取指针
C_BasePlayer *pPrevOwner = pCSWeapon->GetPrevOwner();

	if ( pPrevOwner && pPrevOwner != pPlayer )
	{
		wchar_t wszPlayerName[MAX_PLAYER_NAME_LENGTH];
		g_pVGuiLocalize->ConvertANSIToUnicode( pPrevOwner->GetPlayerName(), wszPlayerName, sizeof( wszPlayerName ) );

		wchar_t wszFinalName[512];
		// 使用 #808080 十六进制颜色控制名字变暗
		V_snwprintf( wszFinalName, ARRAYSIZE( wszFinalName ), L"%ls's%ls", wszPlayerName, wszBaseWepName );
		
		m_weaponPanels[nWepSlot][nWepPos].pNameLabel->SetText( wszFinalName );
	}
	else
	{
		// 自己的枪直接显示原名
		m_weaponPanels[nWepSlot][nWepPos].pNameLabel->SetText( wszBaseWepName );
	}
			...
			for ( int i=0; i < MAX_WEP_SELECT_PANELS; i++ )
			{
				for ( int j=0; j < MAX_WEP_SELECT_POSITIONS; j++ )
				{
					...
					else if ( pNextWeapon && !pPanelWeapon )
					{
						...
						// if it's not a grenade, OR if its a grenade and hasn't been thrown, OR thrown but player still has ammo for it, add it back
