...
//
// Purpose: Authentic CS:GO Style Linear Vertical Fill Progress Bar (Smooth)
//
//=============================================================================//
...
void CHudProgressBar::OnThink()
{
    ...
    else if ( pPlayer->m_bIsDefusing )
    {
        if ( pPlayer->HasDefuser() )
            ...
            newIcon = vgui::CircularProgressBar::ICON_DEFUSE_NOKIT;
    }
    ...
    float flTimeLeft = ( pPlayer->m_flProgressBarStartTime + (float)pPlayer->m_iProgressBarDuration ) - gpGlobals->curtime;
    ...
    float flElapsed = gpGlobals->curtime - pPlayer->m_flProgressBarStartTime;
    float flPercentage = flElapsed / (float)pPlayer->m_iProgressBarDuration;
...
void CircularProgressBar::SetActionIcon( ActionIcon icon )
{
    ...
    {
        case ICON_DEFUSE:
            pszIconPath = "materials/vgui/hud/svg/defuser.svg";
            break;
        case ICON_DEFUSE_NOKIT:
            pszIconPath = "materials/vgui/hud/svg/c4.svg";
            break;
        case ICON_HOSTAGE:
    ...
    int iconSize = (int)(innerDiameter * 0.78f); 
...
void CircularProgressBar::Paint()
{
    ...
    surface()->DrawSetColor( Color( 0, 0, 0, 169 ) );
    DrawFilledRing( centerX, centerY, innerRadius, 0, 0.0f, 360.0f, Color( 0, 0, 0, 169 ) );
    
    DrawFilledRing( centerX, centerY, outerRadius, innerRadius, 0.0f, 360.0f, Color( 128, 128, 128, 200 ) );
    ...
    if ( m_flProgress > 0.0f )
    {
        ...
        int clipTop    = (int)( tall * ( 1.0f - m_flProgress ) );
        int clipBottom = tall;
        int clipLeft   = 0;
        int clipRight  = wide;

        int absLeft   = screenX + clipLeft;
        int absTop    = screenY + clipTop;
        int absRight  = screenX + clipRight;
        int absBottom = screenY + clipBottom;

        int oldLeft, oldTop, oldRight, oldBottom;
        bool bOldClippingDisabled;
        g_pMatSystemSurface->GetClippingRect( oldLeft, oldTop, oldRight, oldBottom, bOldClippingDisabled );

        g_pMatSystemSurface->DisableClipping( false );
        g_pMatSystemSurface->SetClippingRect( absLeft, absTop, absRight, absBottom );

        DrawFilledRing( centerX, centerY, outerRadius, innerRadius, 0.0f, 360.0f, m_FgColor );

        g_pMatSystemSurface->SetClippingRect( oldLeft, oldTop, oldRight, oldBottom );
        g_pMatSystemSurface->DisableClipping( bOldClippingDisabled );
    }
}
...
void CircularProgressBar::ApplySettings( KeyValues *inResourceData )
{
    ...
    m_iRadius = inResourceData->GetInt( "radius", 50 );
    m_iThickness = inResourceData->GetInt( "thickness", 10 );
}
