...
DECLARE_HUDELEMENT( CHudRadarLocation );
DECLARE_HUD_MESSAGE( CHudRadarLocation, UpdateRadar );

static CHudRadarLocation *s_Radar = NULL;
CUtlVector<CPlayerRadarFlash> g_RadarFlashes;


CHudRadarLocation::CHudRadarLocation( const char *pName ) :	vgui::Panel( NULL, "HudRadar" ), CHudElement( pName )
{
...
CHudRadarLocation::~CHudRadarLocation()
{
	s_Radar = NULL;
}


void CHudRadarLocation::Init()
{
	HOOK_HUD_MESSAGE( CHudRadarLocation, UpdateRadar );
}

void CHudRadarLocation::LevelInit()
{
	m_flNextBombFlashTime = 0.0;
	m_bBombFlash = true;
...
void CHudRadarLocation::MsgFunc_UpdateRadar(bf_read &msg )
{
...
bool CHudRadarLocation::ShouldDraw()
{
...
void CHudRadarLocation::SetVisible(bool state)
{
...
void CHudRadarLocation::Paint()
{
	// We are the hud element still, but Overview is in charge of the new style now.
	return;
}

void CHudRadarLocation::WorldToRadar( const Vector location, const Vector origin, const QAngle angles, float &x, float &y, float &z_delta )
{
...
void CHudRadarLocation::DrawPlayerOnRadar( int iPlayer, C_CSPlayer *pLocalPlayer )
{
	float x, y, z_delta;
	...
	if( pCSPR->HasC4( iPlayer ) || bOppositeTeams )
	{
		r = 250; g = 0; b = 0;
	}
...
void CHudRadarLocation::DrawEntityOnRadar( CBaseEntity *pEnt, C_CSPlayer *pLocalPlayer, int flags, int r, int g, int b, int a )
{
	float x, y, z_delta;
	int iBaseDotSize = 4;
...
void CHudRadarLocation::FillRect( int x, int y, int w, int h )
{
...
void CHudRadarLocation::DrawRadarDot( int x, int y, float z_diff, int iBaseDotSize, int flags, int r, int g, int b, int a )
{
...
{
	(GET_HUDELEMENT( CHudRadarLocation ))->DrawRadar();
}

CON_COMMAND( hideradar, "Hides HUD radar" )
{
	(GET_HUDELEMENT( CHudRadarLocation ))->HideRadar();
}
