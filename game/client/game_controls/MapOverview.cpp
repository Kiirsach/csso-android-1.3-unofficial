...
void CMapOverview::FireGameEvent( IGameEvent *event )
{
	...
	else if ( Q_strcmp(type, "bomb_planted") == 0 )
    {
        m_bBombPlanted = true;
    }
    else if ( Q_strcmp(type, "bomb_defused") == 0 || Q_strcmp(type, "bomb_exploded") == 0 )
    {
        m_bBombPlanted = false;
    }
}
...
void CMapOverview::DrawPlantedC4Border()
{
	// ===== 1. 玩家死亡或处于观察者视角时，隐藏（已按要求注释掉） =====
	/*
	...
	*/
	...
	// 未找到实体、已被拆除、实体休眠
	if ( !pC4 || pC4->m_bBombDefused || pC4->IsDormant() )
		return;

	// 爆炸预警激活时隐藏边框（同步图标隐藏逻辑）
	if ( pC4->m_bExplodeWarning )
		return;

	// 超过爆炸时间（已爆炸/回合结束），立刻隐藏边框
	if ( gpGlobals->curtime >= pC4->m_flC4Blow )
		return;

	// ===== 3. 透明度与闪烁逻辑 =====
	// 默认 Alpha 255，当触发 m_flNextGlow 脉冲时降低至 128
	int iAlpha = 255;
	...
	static ConVarRef cl_radar_square( "cl_radar_square" );

	if ( cl_radar_square.IsValid() )
	{
		int iSquareMode = cl_radar_square.GetInt();
		if ( iSquareMode == 1 ) 
		{
			// 设置为 1 时，雷达始终为正方形
			bIsSquare = true;
		}
		else if ( iSquareMode != 0 )
		{
			// 检查按下 TAB 打开计分板（计分板打开时雷达切换为正方形）
			if ( g_pClientMode && g_pClientMode->GetViewport() )
			{
				CCSClientScoreBoardDialog *pScoreboard = dynamic_cast<CCSClientScoreBoardDialog*>( g_pClientMode->GetViewport()->FindChildByName( "scores" ) );
	...
	if ( bIsSquare )
	{
		...
		{
			// 沿着面板边缘向内绘制逐层矩形框
			surface()->DrawOutlinedRect( t, t, iWide - t, iTall - t );
		}
	}
	else
	{
		// ---------------- 圆形雷达边框 ----------------
		int iCenterX = iWide / 2;
		int iCenterY = iTall / 2;
		int iRadius  = min( iWide, iTall ) / 2 - 1;
		const int iSegments = 60; // 圆形拟合段数
		...
		for ( int t = 0; t < iThickness; ++t )
		{
			...
			{
				float a1 = ( (float)i / iSegments ) * M_PI * 2.0f;
				float a2 = ( (float)(i + 1) / iSegments ) * M_PI * 2.0f;

				int x1 = iCenterX + (int)( cosf(a1) * r );
				int y1 = iCenterY + (int)( sinf(a1) * r );
				int x2 = iCenterX + (int)( cosf(a2) * r );
				int y2 = iCenterY + (int)( sinf(a2) * r );
