...
int CCSPlayer::AddDeathmatchKillScore( int nScore, CSWeaponID wepID, bool bIsAssist, const char* szVictim )
{
	...
	// ===== 修复并解开奖励得分逻辑 =====
	if ( CSGameRules()->IsDMBonusActive() && !bIsAssist )
	{
		// 1. 判断当前击杀武器是否为奖励武器 (推荐直接对比 CSWeaponID)
		if ( CSGameRules()->GetDMBonusWeaponID() == wepID )
		{
			// 方案 A：固定加分（例如每杀一人固定额外 +6 分，如 CS:GO 默认）
			nBonus = 6; 

			// 方案 B：按百分比加分（原注释逻辑）
			/*
			static ConVarRef mp_dm_bonus_percent( "mp_dm_bonus_percent" );
			if ( mp_dm_bonus_percent.IsValid() )
			{
